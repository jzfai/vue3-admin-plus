...
const formString = JSON.stringify(queryParams)
/** 查询授权用户列表 */
function getList() {
  loading.value = true
...
/** 重置按钮操作 */
function resetQuery() {
  resetData(queryParams, formString)
  getList()
}
// 多选框选中数据
...
function cancelAuthUser(row) {
  elConfirm('确认', `确认要取消该用户"${row.userName}"角色吗？`)
    .then(() => {
      return authUserCancel({ userId: row.userId, roleId: queryParams.roleId })
    })
    .then(() => {
      getList()
      ElMessage({ message: '取消授权成功', type: 'success' })
    })
    .catch(() => {})
}
/** 批量取消授权按钮操作 */
