    ...
    items: [
      {
        ...
        title: '名字',
        span: 8,
        titlePrefix: { message: '可自定义任何内容', icon: 'vxe-icon-question-circle-fill' },
    ...
    ajax: {
      ...
      query: async ({ page, sorts, filters, form }) => {
        ...
        const response = await fetch(
          `${serveApiUrl.value}/api/pub/page/list/${page.pageSize}/${page.currentPage}?${XEUtils.serialize(
            queryParams
          )}`
        )
      ...
      delete: async ({ body }) => {
        const response = await fetch(`${serveApiUrl.value}/api/pub/save`, {
          method: 'POST',
